...
class TestArchiveKeywords(unittest.TestCase):
    ...
    def test_create_zip_with_default_compression(self, mock_walk, mock_zipfile):
        ...
        mock_zipfile.ZipFile().write.assert_called_once_with(
            "/foo/file1.txt", arcname="file1.txt"
        )
    ...
    def test_create_zip_with_no_default_compression(self, mock_walk, mock_zipfile):
        ...
        archive_keywords = ArchiveKeywords()
        archive_keywords.create_zip_from_files_in_directory(
            "/foo", "filename.zip", False, "lzma"
        )

        mock_zipfile.ZipFile.assert_called_once_with(
            "filename.zip", "w", zipfile.ZIP_LZMA
        )
        mock_zipfile.ZipFile().write.assert_called_once_with(
            "/foo/file1.txt", arcname="file1.txt"
        )
    ...
    def test_create_tar_from_files_in_directory(self, mock_walk, mock_tarfile):
        ...
        mock_tarfile.open().add.assert_called_once_with(
            "/foo/file1.txt", arcname="file1.txt"
        )
    ...
    def test_zip_archive_should_contain_file(
        self, mock_oslib, mock_zipfile, mock_collections
    ):
        ...
        mock_collections().list_should_contain_value.assert_called_once_with(
            ["file1.txt", "file2.txt"], "filename.zip"
        )
    ...
    @mock.patch("ArchiveLibrary.keywords.zipfile")
    @mock.patch("ArchiveLibrary.keywords.OperatingSystem")
    def test_no_zip_archive_should_contain_file(
        self, mock_oslib, mock_zipfile, mock_tarfile, mock_collections
    ):
        mock_oslib.file_should_exist = Mock()
